...
- Name: 01-auth.spec.js >> 认证模块 >> 注册页面 >> 应正确渲染注册页面元素
- Location: tests\01-auth.spec.js:69:5
...
Locator: getByText('注册')
Expected: visible
Error: strict mode violation: getByText('注册') resolved to 2 elements:
    1) <div class="adm-nav-bar-title">注册</div> aka getByText('注册').first()
    2) <span>注册</span> aka getByRole('button', { name: '注册' })

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for getByText('注册')
